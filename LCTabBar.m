...
- (void)addBigTabBarItem:(UITabBarItem *)item {
    
    LCBigTabBarItem *tabBarItem = [[LCBigTabBarItem alloc] initWithItemImageRatio:self.itemImageRatio];
    tabBarItem.badgeTitleFont         = self.badgeTitleFont;
    tabBarItem.itemTitleFont          = self.itemTitleFont;
    tabBarItem.itemTitleColor         = self.itemTitleColor;
    tabBarItem.selectedItemTitleColor = self.selectedItemTitleColor;
...
//- (void)addBigCustomeTabBarItem:(UITabBarItem *)item {
//    
//    LCBigTabBarItem *tabBarItem = [[LCBigTabBarItem alloc] init
//    tabBarItem.badgeTitleFont         = self.badgeTitleFont;
//    tabBarItem.itemTitleFont          = self.itemTitleFont;
//    tabBarItem.itemTitleColor         = self.itemTitleColor;
//    tabBarItem.selectedItemTitleColor = self.selectedItemTitleColor;
...
- (void)layoutSubviews {
    ...
    for (int index = 0; index < count; index++) {
        ...
        if (index == 2)
        {
            tabBarItem.frame = CGRectMake(ScreenWidth/2-64/2, -(64-49), 64, 64);
        }
        else
        {
